...
app.post("/signup/:key", function (req, res) {
  ...
  dbUsers.create(req.body).then(function (data) {
    dbSockets.create({
      "socketId": "",
      "name": req.body.name,
      ...
      let options = {
        ...
        html: emails.emitEmailHtml("Thank you for signing up to Zone. You can now login to start connecting with your friends and loved ones. <br> Get in your zone!")
      }
...
app.route("/users/:key/:token/:username?").get(function (req, res) {
  if (req.params.username) {
    let startFinding = dbUsers.findOne({ username: req.params.username }).populate("tweets retweets likedTweets").populate({
      path: "following",
    ...
    let startFinding = dbUsers.findById(req.query.id).populate("tweets retweets likedTweets").populate({
      path: "following",
      select: "_id username name photo"
    }).populate({
      path: "followers",
...
app.route("/minorusers/:key/:token/:username").get(function (req, res) {
  ...
  let startFinding = dbUsers.findOne({ username: req.params.username }).select("photo name username")
...
app.get("/pass/:key/:email", function (req, res) {
  dbUsers.findOne({ email: req.params.email }).then(function (data) {
    if (!data) return errorResponce(res, "User does not exist")
...
app.post("/updatePass/:token", function (req, res) {
  ...
  jwt.verify(token, process.env.jwtkey, function (err, obj) {
    ...
    dbUsers.findOneAndUpdate({ email: userEmail }, { password: newPass }, {
      runValidators: true
    }).then(function () {
      res.status(200).header({
        "content-type": "application/json"
